<template>
  ...
  >
    ...
    <span
      class="absolute top-0 right-0 w-1/2 h-4 border-t-4 border-r-4 border-primary rounded-tr-2xl"
    ></span>
    <span
      class="absolute bottom-0 left-0 w-1/2 h-4 border-b-4 border-l-4 border-secondary rounded-bl-2xl"
    ></span>
    <!-- Background shapes -->
    <div
      class="absolute -left-20 top-0 flex flex-col gap-10 opacity-20 pointer-events-none"
    >
      <span class="w-16 h-[332px] rounded-full bg-primary blur-2xl"></span>
      <span class="w-16 h-[332px] rounded-full bg-[#f88fc2] blur-2xl"></span>
    </div>
    ...
    <div class="flex flex-row flex-wrap gap-8 justify-center">
      <div
        ...
        :key="index"
        class="relative bg-[#111A3E] border border-[#1f1641] rounded-xl shadow-lg hover:shadow-xl transition-shadow p-6 max-w-xs flex flex-col justify-between"
      >
